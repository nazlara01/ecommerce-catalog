    ...
    <div class="product-description">
      ...
      <div class="gender-and-rate-wrapper">
        ...
        <div class="rate-content">
          ...
          <span
            ...
            class="circle"
            :class="{
              'man-circle': value < $store.state.dataProduct?.rating?.rate && !$store.state.dataProduct?.category?.includes('women'),
              'women-circle': value < $store.state.dataProduct?.rating?.rate && $store.state.dataProduct?.category?.includes('women'),
            }"
          ></span>
        </div>
      </div>
      ...
      <div class="buttons-container">
        <button
          class="buy-button"
          :class="{
            'women-buy-button': $store.state.dataProduct?.category.includes('women'),
            'man-buy-button': !$store.state.dataProduct?.category.includes('women'),
          }"
        >
          Buy now
        </button>
        <button
          ...
          :class="{
            'women-text': $store.state.dataProduct?.category.includes('women'),
            'women-border-next-button': $store.state.dataProduct?.category.includes('women'),
            'man-text': !$store.state.dataProduct?.category.includes('women'),
            'man-border-next-button': !$store.state.dataProduct?.category.includes('women'),
          }"
          @click="fetchData"
        >
          Next product
